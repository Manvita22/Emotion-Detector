"""Train an MLP classifier on pre-extracted text embeddings."""
import argparse
import numpy as np
...
def main():
    ...
    p.add_argument('--features', default='features/text_emb.npy')
    ...
    Path('models/dl_text_pipeline').mkdir(parents=True, exist_ok=True)
    torch.save({'model_state': model.state_dict(), 'label_classes': le.classes_.tolist()}, 'models/dl_text_pipeline/text_mlp.pt')
    print('Saved model to models/dl_text_pipeline/text_mlp.pt')
